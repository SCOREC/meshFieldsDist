...
int main(int argc, char** argv) {
  ...
  Ctrlr c({mesh.nverts(), mesh.nverts(), 10});
  Kokkos::Timer timer;
  ...
  auto setVtx = KOKKOS_LAMBDA (const int i) {
    vtxRankId(i / 10, i % 10) = rank;
  };
  mf.parallel_for({0},{mesh.nverts() * 10}, setVtx, "set_vertex");
  ...
  double error = 0;
  Kokkos::parallel_reduce ("Reduction", numVerts * 10, KOKKOS_LAMBDA (const int i, double& update) {
	  update += abs(owners[i / 10] - vtxRankId(i / 10, i % 10));
  }, error);
  if(error != 0)
